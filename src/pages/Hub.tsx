...
// ─── Hardcoded Supabase Client (isolated from main app) ────────────────────────
...
// ─── Component ─────────────────────────────────────────────────────────────────
const Hub = () => {
  const [broadcast, setBroadcast] = useState<HubBroadcast | null>(null);
  ...
  const statCards = [
    { icon: Milestone, value: stats ? `${fmt(stats.total_distance)} km` : '—', label: 'Total Distance', color: 'text-primary' },
    { icon: Truck, value: stats ? fmt(stats.total_deliveries) : '—', label: 'Deliveries', color: 'text-primary' },
    { icon: DollarSign, value: stats ? fmtCurrency(stats.total_income) : '—', label: 'Revenue', color: 'text-yellow-400' },
    { icon: Users, value: stats ? String(stats.active_drivers) : '—', label: 'Active Drivers', color: 'text-emerald-400' },
    { icon: Fuel, value: stats ? `${fmt(stats.total_fuel)} L` : '—', label: 'Fuel Consumed', color: 'text-orange-400' },
    { icon: TrendingUp, value: stats ? fmtCurrency(stats.total_profit) : '—', label: 'Net Profit', color: 'text-green-400' },
  ];
        ...
        <div className="container mx-auto px-4 py-12 md:py-20 relative">
          ...
          <section className="mb-32">
            <div className="flex items-center justify-between mb-8">
              ...
              {broadcast?.last_updated && (
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest">
                  Updated {new Date(broadcast.last_updated).toLocaleTimeString()}
                </p>
              )}
            </div>
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          ...
                          <p className="text-primary text-[10px] uppercase tracking-tighter font-display">
                            {fmtCurrency(driver.total_earnings)} earned
                          </p>
                        </div>
                      </div>
