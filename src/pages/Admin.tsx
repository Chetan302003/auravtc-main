...
const Admin = () => {
  const { user, isAdmin, loading, signOut, hasRole } = useAuth();
    ...
    <Layout>
      <div className="min-h-screen pt-20 sm:pt-24 pb-8 sm:pb-12 px-4 relative">
        ...
        <div className="container mx-auto max-w-7xl relative z-10">
          ...
          >
            <div>
              ...
              <p className="text-xs sm:text-sm text-muted-foreground truncate">
                ...
                {!isAdmin && hasRole('hr') && <span className="ml-2 text-blue-400">(HR)</span>}
                {!isAdmin && hasRole('media') && <span className="ml-2 text-purple-400">(Media)</span>}
                {!isAdmin && hasRole('event') && <span className="ml-2 text-yellow-400">(Event)</span>}
                {!isAdmin && hasRole('driver') && <span className="ml-2 text-muted-foreground">(Driver)</span>}
                {!isAdmin && !hasRole('hr') && !hasRole('media') && !hasRole('event') && !hasRole('driver') && (
          ...
          {/* Main Content Grid - Visible to Admin and HR */}
          {(isAdmin || hasRole('hr')) && (
          ...
          {/* Dashboard Analytics - Visible to Admin only */}
          {isAdmin && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* VTC Settings Management - Visible to Admin only */}
          {isAdmin && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* Slot Management - Visible to Admin and Event role */}
          ...
          {/* Gallery Management - Visible to Admin and Media role */}
          {(isAdmin || hasRole('media')) && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* User Management - Visible to Admin and HR role */}
          {(isAdmin || hasRole('hr')) && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* System Logs - Visible to Admin only */}
          {isAdmin && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
